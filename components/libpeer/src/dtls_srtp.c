...
void dtls_srtp_decrypt_rtp_packet(DtlsSrtp* dtls_srtp, uint8_t* packet, int* bytes) {
  ...
  srtp_unprotect(dtls_srtp->srtp_in, packet, *bytes, packet, &out_len);
  *bytes = out_len;
}

void dtls_srtp_decrypt_rtcp_packet(DtlsSrtp* dtls_srtp, uint8_t* packet, int* bytes) {
  size_t out_len = *bytes;
  srtp_unprotect_rtcp(dtls_srtp->srtp_in, packet, *bytes, packet, &out_len);
  *bytes = out_len;
}

void dtls_srtp_encrypt_rtp_packet(DtlsSrtp* dtls_srtp, uint8_t* packet, int* bytes) {
  size_t out_len = *bytes;
  srtp_protect(dtls_srtp->srtp_out, packet, *bytes, packet, &out_len, 0);
  *bytes = out_len;
}

void dtls_srtp_encrypt_rctp_packet(DtlsSrtp* dtls_srtp, uint8_t* packet, int* bytes) {
  size_t out_len = *bytes;
  srtp_protect_rtcp(dtls_srtp->srtp_out, packet, *bytes, packet, &out_len, 0);
  *bytes = out_len;
}
