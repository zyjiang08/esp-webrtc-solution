...
#define SDP_STREAM_ID "iot"
#define SDP_AUDIO_TRACK_ID "audio0"
#define SDP_VIDEO_TRACK_ID "video0"
...
void sdp_append_h264(char* sdp) {
  ...
  sdp_append(sdp, "a=msid:%s %s", SDP_STREAM_ID, SDP_VIDEO_TRACK_ID);
  sdp_append(sdp, "a=ssrc:1 cname:webrtc-h264");
  sdp_append(sdp, "a=ssrc:1 msid:%s %s", SDP_STREAM_ID, SDP_VIDEO_TRACK_ID);
  sdp_append(sdp, "a=ssrc:1 mslabel:%s", SDP_STREAM_ID);
  sdp_append(sdp, "a=ssrc:1 label:%s", SDP_VIDEO_TRACK_ID);
...
void sdp_append_pcma(char* sdp) {
  ...
  sdp_append(sdp, "a=ptime:20");
  sdp_append(sdp, "a=maxptime:20");
  sdp_append(sdp, "a=msid:%s %s", SDP_STREAM_ID, SDP_AUDIO_TRACK_ID);
  sdp_append(sdp, "a=ssrc:4 cname:webrtc-pcma");
  sdp_append(sdp, "a=ssrc:4 msid:%s %s", SDP_STREAM_ID, SDP_AUDIO_TRACK_ID);
  sdp_append(sdp, "a=ssrc:4 mslabel:%s", SDP_STREAM_ID);
  sdp_append(sdp, "a=ssrc:4 label:%s", SDP_AUDIO_TRACK_ID);
  sdp_append(sdp, "a=sendrecv");
...
void sdp_append_pcmu(char* sdp) {
  ...
  sdp_append(sdp, "a=ssrc:5 msid:%s %s", SDP_STREAM_ID, SDP_AUDIO_TRACK_ID);
  sdp_append(sdp, "a=ssrc:5 mslabel:%s", SDP_STREAM_ID);
  sdp_append(sdp, "a=ssrc:5 label:%s", SDP_AUDIO_TRACK_ID);
  sdp_append(sdp, "a=sendrecv");
  sdp_append(sdp, "a=mid:audio");
  sdp_append(sdp, "a=rtcp-mux");
}
...
void sdp_append_opus(char* sdp) {
  ...
  sdp_append(sdp, "a=ssrc:6 msid:%s %s", SDP_STREAM_ID, SDP_AUDIO_TRACK_ID);
  sdp_append(sdp, "a=ssrc:6 mslabel:%s", SDP_STREAM_ID);
  sdp_append(sdp, "a=ssrc:6 label:%s", SDP_AUDIO_TRACK_ID);
  sdp_append(sdp, "a=sendrecv");
  sdp_append(sdp, "a=mid:audio");
  sdp_append(sdp, "a=rtcp-mux");
}
