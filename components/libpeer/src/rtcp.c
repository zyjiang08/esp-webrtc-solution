...
int rtcp_get_pli(uint8_t* packet, int len, uint32_t ssrc) {
  ...
  rtcp_header_set_version(rtcp_header, 2);
  rtcp_header_set_padding(rtcp_header, 0);
  rtcp_header_set_rc(rtcp_header, 1);
...
int rtcp_get_fir(uint8_t* packet, int len, int* seqnr) {
  ...
  *seqnr = *seqnr + 1;
  ...
  rtcp->flags = 0;
  rtcp->type = RTCP_PSFB;
  rtcp_header_set_version(rtcp, 2);
  rtcp_header_set_padding(rtcp, 0);
  rtcp_header_set_rc(rtcp, 4);
