...
int rtp_packet_validate(uint8_t* packet, size_t size) {
  ...
  uint8_t version = rtp_header_get_version(rtp_header);
  uint8_t payload_type = rtp_header_get_payload_type(rtp_header);
  return (version == 2) && ((payload_type < 64) || (payload_type >= 96));
}
...
static int rtp_encoder_encode_h264_single(RtpEncoder* rtp_encoder, uint8_t* buf, size_t size) {
  ...
  rtp_packet->header.flags = 0;
  rtp_packet->header.payload_type = 0;
  rtp_header_set_version(&rtp_packet->header, 2);
  rtp_header_set_padding(&rtp_packet->header, 0);
  rtp_header_set_extension(&rtp_packet->header, 0);
  rtp_header_set_csrc_count(&rtp_packet->header, 0);
  rtp_header_set_marker(&rtp_packet->header, 0);
  rtp_header_set_payload_type(&rtp_packet->header, rtp_encoder->type);
  ...
  if ((*buf & 0x1f) == 0x05 || (*buf & 0x1f) == 0x01) {
    rtp_header_set_marker(&rtp_packet->header, 1);
    rtp_encoder->timestamp += rtp_encoder->timestamp_increment;
  }
#if 0
  LOGI("markbit: %d, timestamp: %d, nalu type: %d", rtp_header_get_marker(&rtp_packet->header), rtp_encoder->timestamp, buf[0] & 0x1f);
#endif
...
static int rtp_encoder_encode_generic(RtpEncoder* rtp_encoder, uint8_t* buf, size_t size) {
  ...
  rtp_header_set_version(rtp_header, 2);
  rtp_header_set_padding(rtp_header, 0);
  rtp_header_set_extension(rtp_header, 0);
  rtp_header_set_csrc_count(rtp_header, 0);
  rtp_header_set_marker(rtp_header, 0);
  rtp_header_set_payload_type(rtp_header, rtp_encoder->type);
